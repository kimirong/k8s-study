...
```bash
# 写数据
kubectl exec <pod> -- mysql -uroot -p... -e "CREATE TABLE mydb.users(...); INSERT ...; SELECT * FROM mydb.users;"

# 删 Pod
kubectl delete pod <pod>

# 新 Pod 起来后，数据还在
kubectl exec <新pod> -- mysql -uroot -p... -e "SELECT * FROM mydb.users;"

# 真实数据文件在节点上
ls /data/mysql/mydb/     # users.ibd 等，Pod 之外
```
...
1. **镜像分发**：mysql:8.0 从 daocloud 拉，worker1 拉得极慢（7KB/s），解决：master 上 export 成 tar → 内网 scp → import。以后大镜像都用这套。
